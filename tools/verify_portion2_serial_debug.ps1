...
$checks = @(
    ...
    @{ Name = "portion2 planned-point arrive threshold is relaxed"; Pass = $guandaoC -match "PORTION2_PURSUIT_THRESHOLD\s+0\.18f" },
    ...
    @{ Name = "portion2 final raw point stop is position tight"; Pass = $guandaoC -match "PORTION2_FINAL_RAW_POINT_STOP_DIST\s+0\.25f" -and $guandaoC -match "raw_point\s*>=\s*raw_length\s*-\s*1" -and $guandaoC -match "dist_to_final\s*<=\s*PORTION2_FINAL_RAW_POINT_STOP_DIST" },
    ...
    @{ Name = "portion2 reference points are smoothed before planning"; Pass = $guandaoC -match "portion2_smooth_reference_route\s*\(" -and $guandaoC -match "PORTION2_REFERENCE_SMOOTH_PASSES" },
    @{ Name = "portion2 pursuit uses softer steering"; Pass = $guandaoC -match "PORTION2_STEERING_CMD_LIMIT\s+20\.0f" -and $guandaoC -match "PORTION2_STEERING_GAIN\s+1\.25f" -and $guandaoC -match "PORTION2_STEER_RATE_LIMIT\s+1\.0f" },
    @{ Name = "portion2 rear half uses softer steering"; Pass = $guandaoC -match "PORTION2_FINAL_SECTION_STEERING_LIMIT\s+15\.0f" -and $guandaoC -match "state->current_point_index\s*>\s*route_length\s*-\s*50" },
    @{ Name = "portion2 run final yaw is not cleared before end log"; Pass = $guandaoC -notmatch "current_point_index\s*>=\s*guandao_route_length\s*\(&portion_2\)[\s\S]{0,280}portion2_run_final_yaw\s*=\s*0\.0f" },
