...
$checks = @(
    ...
    @{ Name = "record gps rejection reasons cover every filter"; Pass = $guandaoC -match "PORTION2_GPS_REJECT_NO_FIX" -and $guandaoC -match "PORTION2_GPS_REJECT_LOW_SAT" -and $guandaoC -match "PORTION2_GPS_REJECT_ZERO_COORD" -and $guandaoC -match "PORTION2_GPS_REJECT_INTERVAL" -and $guandaoC -match "PORTION2_GPS_REJECT_REPEAT" -and $guandaoC -match "PORTION2_GPS_REJECT_JUMP" -and $guandaoC -match "PORTION2_GPS_REJECT_CAPACITY" -and $guandaoC -match "PORTION2_GPS_REJECT_INDEX" },
    @{ Name = "record gps rejection serial log exposes live gnss state"; Pass = $guandaoC -match "\[P2-REC-GPS-SKIP\]" -and $guandaoC -match "reason=%s fix=%u sats=%u hdop100=%u coord=%u seq=%lu gps=%u/%u" },
    ...
    @{ Name = "gnss gga parser exposes hdop"; Pass = $gnssH -match "float\s+hdop" -and $gnssC -match "gnss->hdop\s*=\s*get_float_number\s*\(\s*&buf\[get_parameter_index\(8" -and $gpsC -match "hdop100=" },
    @{ Name = "gps recording rejects poor hdop"; Pass = $guandaoC -match "PORTION2_GPS_MAX_HDOP\s+\(2\.5f\)" -and $guandaoC -match "PORTION2_GPS_REJECT_BAD_HDOP" -and $guandaoC -match "gnss\.hdop\s*<=\s*0\.0f\s*\|\|\s*gnss\.hdop\s*>\s*PORTION2_GPS_MAX_HDOP" },
    @{ Name = "gps recording and fusion accept six satellites"; Pass = $guandaoC -match "PORTION2_GPS_RECORD_MIN_SATELLITES\s+6U" -and $gpsC -match "PORTION2_GPS_FUSION_MIN_SATELLITES\s+\(6U\)" },
    @{ Name = "gps startup requires five tightly stable fixes"; Pass = $gpsC -match "PORTION2_GPS_STARTUP_REQUIRED_SAMPLES\s+\(5U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_STABILITY_M\s+\(0\.6f\)" -and $gpsC -match "PORTION2_GPS_STARTUP_MAX_SHIFT_M\s+\(3\.0f\)" },
    @{ Name = "gps fusion gates hdop and excessive startup shift"; Pass = $gpsC -match "PORTION2_GPS_FUSION_MAX_HDOP\s+\(2\.5f\)" -and $gpsC -match "gnss\.hdop\s*<=\s*0\.0f\s*\|\|\s*gnss\.hdop\s*>\s*PORTION2_GPS_FUSION_MAX_HDOP" -and $gpsC -match "startup_shift\s*>\s*PORTION2_GPS_STARTUP_MAX_SHIFT_M" },
    @{ Name = "gps fusion recovers after three good fixes"; Pass = $gpsC -match "PORTION2_GPS_RECOVERY_REQUIRED_FIXES\s+\(3U\)" -and $gpsC -match "PORTION2_GPS_RECOVERY_MAX_ERROR\s+\(2\.0f\)" -and $gpsC -match "recovery_good_count\+\+" -and $gpsC -match "recovery_good_count\s*>=\s*PORTION2_GPS_RECOVERY_REQUIRED_FIXES[\s\S]*?ready\s*=\s*1" },
    ...
    @{ Name = "runtime raw-point tracking event emitted"; Pass = $guandaoC -match "\[P2-TRACK\]" },
    ...
    @{ Name = "route tracking uses thirty centimeter bad threshold"; Pass = $guandaoC -match "PORTION2_TRACK_BAD_THRESHOLD_M\s+0\.30f" },
    @{ Name = "route tracking projects position onto a clamped segment"; Pass = $guandaoC -match "portion2_track_sample_t" -and $guandaoC -match "dot\s*=\s*offset_x\s*\*\s*segment_dx\s*\+\s*offset_y\s*\*\s*segment_dy" -and $guandaoC -match "projection\s*=\s*dot\s*/\s*segment_length_sq" -and $guandaoC -match "if\s*\(projection\s*<\s*0\.0f\)\s*projection\s*=\s*0\.0f" -and $guandaoC -match "if\s*\(projection\s*>\s*1\.0f\)\s*projection\s*=\s*1\.0f" -and $guandaoC -match "cross\s*=\s*segment_dx\s*\*\s*offset_y\s*-\s*segment_dy\s*\*\s*offset_x" },
    @{ Name = "route tracking emits one event per raw point"; Pass = $guandaoC -match "raw_point\s*==\s*portion2_run_last_report_point" -and $guandaoC -match "portion2_run_last_report_point\s*=\s*raw_point" -and $guandaoC -match "\[P2-TRACK\]" -and $guandaoC -notmatch "\[P2-RUN-PT\]" },
    @{ Name = "route tracking line exposes deviation heading steering and gps"; Pass = $guandaoC -match "side=%s off=" -and $guandaoC -match "head_err=" -and $guandaoC -match "steer=" -and $guandaoC -match "final=" -and $guandaoC -match "gps=%s gps_err=" -and $guandaoC -match "status=%s" },
    @{ Name = "route tracking heading follows reverse motion direction"; Pass = $guandaoC -match "track_heading\s*=\s*portion2_run_drive_reverse\s*\?\s*Yaw_1\s*\+\s*180\.0f\s*:\s*Yaw_1" -and $guandaoC -match "sample\.heading_error\s*=\s*guandao_normalize_angle\s*\(guandao_segment_yaw\([^;]*-\s*track_heading\)" },
    @{ Name = "route tracking records first and maximum bad points"; Pass = $guandaoC -match "portion2_track_first_bad_raw" -and $guandaoC -match "portion2_track_max_raw" -and $guandaoC -match "portion2_track_max_off" -and $guandaoC -match "sample\.abs_error\s*>=\s*PORTION2_TRACK_BAD_THRESHOLD_M" },
    @{ Name = "route tracking emits end summary on finish and stop"; Pass = $guandaoC -match "\[P2-TRACK-END\]" -and $guandaoC -match "first_bad=%d max_raw=%d max_off=" -and $guandaoC -match "portion2_serial_log_track_summary\s*\(" -and $guandaoC -match "void\s+portion2_run_stop\s*\([^)]*\)[\s\S]*?portion2_serial_log_track_summary\s*\(" },
    @{ Name = "runtime detailed serial status is limited to one hertz"; Pass = $guandaoC -match "static\s+void\s+portion2_serial_log_run\s*\([^)]*\)[\s\S]*?now_ms\s*-\s*last_ms\)\s*<\s*1000U" },
    @{ Name = "portion2 route yaw alignment helper exists"; Pass = $guandaoC -match "static\s+float\s+portion2_align_route_to_current_yaw\s*\(" -and $guandaoC -match "run_start_theta\s*=\s*portion2_run_drive_reverse\s*\?\s*Yaw_1\s*\+\s*180\.0f\s*:\s*Yaw_1" },
    ...
    @{ Name = "record K4 short clears release flag before run mode"; Pass = $guandaoC -match "k4_event\s*==\s*PORTION2_MODE_KEY_SHORT[\s\S]*?portion2_record_key_state_reset\s*\(\s*\)[\s\S]*?main_mode\s*=\s*Guandao_Voice" },
    @{ Name = "K4 long press is locked by shared mode key code"; Pass = $guandaoC -match "portion2_mode_k4_event\s*\(\s*void\s*\)[\s\S]*?gpio_get_level\s*\(\s*KEY4\s*\)\s*==\s*0" -and $guandaoC -match "now_ms\s*-\s*portion2_mode_k4_start_ms\s*\)\s*>=\s*1500U[\s\S]*?portion2_mode_k4_wait_release\s*=\s*1[\s\S]*?return\s+PORTION2_MODE_KEY_LONG" },
    ...
    @{ Name = "mode K4 shared event API declared"; Pass = $guandaoH -match "portion2_mode_key_event_t\s+portion2_mode_k4_event\s*\(\s*void\s*\)" -and $guandaoH -match "void\s+portion2_mode_key_transition_lock\s*\(\s*void\s*\)" },
    @{ Name = "mode K4 shared event waits for release"; Pass = $guandaoC -match "static\s+uint8\s+portion2_mode_k4_wait_release" -and $guandaoC -match "portion2_mode_key_event_t\s+portion2_mode_k4_event\s*\(\s*void\s*\)[\s\S]*?gpio_get_level\s*\(\s*KEY4\s*\)" -and $guandaoC -match "portion2_mode_k4_wait_release\s*=\s*1" },
    @{ Name = "record K4 enters run through shared event"; Pass = $guandaoC -match "k4_event\s*=\s*portion2_mode_k4_event\s*\(\s*\)" -and $guandaoC -match "k4_event\s*==\s*PORTION2_MODE_KEY_SHORT[\s\S]*?portion2_mode_key_transition_lock\s*\(\s*\)[\s\S]*?main_mode\s*=\s*Guandao_Voice" },
    @{ Name = "run K4 returns to record through shared event"; Pass = $cpu0 -match "k4_event\s*=\s*portion2_mode_k4_event\s*\(\s*\)[\s\S]*?k4_event\s*==\s*PORTION2_MODE_KEY_SHORT[\s\S]*?portion2_mode_key_transition_lock\s*\(\s*\)[\s\S]*?portion2_record_enter_mode\s*\(\s*\)" },
    @{ Name = "run to record restores key gpio after screen init"; Pass = $cpu0 -match "k4_event\s*==\s*PORTION2_MODE_KEY_SHORT[\s\S]*?portion2_record_enter_mode\s*\(\s*\)\s*;\s*[\r\n\s]*Key_Init\s*\(\s*\)" },
    ...
    @{ Name = "each route stores up to forty nine inertial points"; Pass = $guandaoH -match "PORTION2_ROUTE_MAX_POINTS\s+49" -and $guandaoH -match "PORTION2_TOTAL_ROUTE_POINTS\s+441" -and $guandaoH -match "MAX_LENGTH_INDEX\s+441" },
    @{ Name = "each route stores up to twenty gps points"; Pass = $guandaoH -match "PORTION2_GPS_PER_ROUTE\s+20" },
    @{ Name = "nine routes reserve 180 gps records"; Pass = $guandaoH -match "MAX_GPS_RECODE\s+180" -and $guandaoH -match "PORTION2_TOTAL_GPS_COUNT\s+180" },
    @{ Name = "route and gps backing arrays have compile time guards"; Pass = $guandaoH -match "PORTION2_TOTAL_ROUTE_POINTS\s*!=\s*PORTION2_ROUTE_COUNT\s*\*\s*PORTION2_ROUTE_MAX_POINTS" -and $guandaoH -match "MAX_LENGTH_INDEX\s*<\s*PORTION2_TOTAL_ROUTE_POINTS" -and $guandaoH -match "MAX_GPS_RECODE\s*<\s*PORTION2_TOTAL_GPS_COUNT" },
    @{ Name = "record spacing remains 0.4m inertial and 1.0m gps"; Pass = $flashC -match "FLASH_RECODE_THRESHOLD_DEFAULT\s+\(0\.4f\)" -and $guandaoC -match "PORTION2_AUTO_GPS_RECORD_DIST\s+1\.0f" },
    ...
    @{ Name = "inertial and gps flash pages have compile time guards"; Pass = $flashC -match "PORTION2_INERTIAL_PAGE_WORDS" -and $flashC -match "PORTION2_INERTIAL_PAGE_WORDS\s*>\s*1020" -and $flashC -match "PORTION2_GPS_PAGE_DATA_INDEX\s*\+\s*PORTION2_TOTAL_GPS_COUNT\s*\*\s*4\s*>\s*1020" },
    @{ Name = "second gps page stores version marker and total count"; Pass = $flashC -match "PORTION2_GPS_LAYOUT_MAGIC\s+\(0x50324754U\)" -and $flashC -match "PORTION2_GPS_PAGE_TOTAL_INDEX" -and $flashC -match "PORTION2_TOTAL_GPS_COUNT" },
    ...
    @{ Name = "gps startup uses bounded stability sampling"; Pass = $gpsC -match "PORTION2_GPS_STARTUP_REQUIRED_SAMPLES\s+\(5U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_TIMEOUT_MS\s+\(10000U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_STABILITY_M\s+\(0\.6f\)" },
    ...
    @{ Name = "new twenty point gps layout marker invalidates old binding"; Pass = $flashC -match "PORTION2_GPS_LAYOUT_MAGIC\s+\(0x50324754U\)" },
    ...
    @{ Name = "encoder yaw actions use bounded PD steering"; Pass = $fixedActionC -match "SUBJECT_2_ENCODER_YAW_KP" -and $fixedActionC -match "SUBJECT_2_ENCODER_YAW_KD" -and $fixedActionC -match "SUBJECT_2_ENCODER_YAW_DEADBAND_DEG\s+\(0\.05f\)" -and $fixedActionC -match "SUBJECT_2_ENCODER_YAW_STEER_LIMIT_DEG" -and $fixedActionC -match "yaw_target\s*-\s*Yaw_Straight_1" },
    ...
    @{ Name = "encoder yaw action stops by distance without total timeout"; Pass = $fixedActionC -match "SUBJECT_2_ENCODER_YAW_SLOWDOWN_M\s+\(1\.0f\)" -and $fixedActionC -notmatch "SUBJECT_2_ENCODER_YAW_TIMEOUT_MS" -and $fixedActionC -notmatch 'subject_2_encoder_yaw_finish\("TIMEOUT"\)' },
    @{ Name = "encoder yaw action retains stall protection"; Pass = $fixedActionC -match "SUBJECT_2_ENCODER_YAW_STALL_MS\s+\(3000U\)" -and $fixedActionC -match 'subject_2_encoder_yaw_finish\("STALL"\)' },
    @{ Name = "straight action has dedicated sensitive yaw"; Pass = $imuH -match "extern\s+float\s+Yaw_Straight_1" -and $imuC -match "float\s+Yaw_Straight_1\s*=\s*0" -and $imuC -match "STRAIGHT_YAW_GYRO_DEADBAND_RAD_S\s+\(0\.005f\)" -and $fixedActionC -match "yaw_target\s*=\s*[^;]*Yaw_Straight_1" },
    ...
    @{ Name = "portion2 records route start and final yaw"; Pass = $guandaoH -match "portion2_route_start_yaw" -and $guandaoH -match "portion2_route_final_yaw" -and $guandaoC -match "portion2_route_start_yaw\[portion2_record_route\]\s*=\s*Yaw_1" -and $guandaoC -match "portion2_route_final_yaw\[portion2_record_route\]\s*=\s*Yaw_1" },
    @{ Name = "portion2 terminal pose is persisted with layout magic"; Pass = $flashC -match "PORTION2_ROUTE_LAYOUT_MAGIC" -and $flashC -match "portion2_route_start_yaw\[i\]" -and $flashC -match "portion2_route_final_yaw\[i\]" -and $flashC -match "PORTION2_ROUTE_METADATA_WORDS\s+\(PORTION2_ROUTE_COUNT\s*\*\s*4\)" },
    @{ Name = "portion2 final stop is tightened to 0.20m and five degrees"; Pass = $guandaoC -match "PORTION2_FINAL_RAW_POINT_STOP_DIST\s+0\.20f" -and $guandaoC -match "PORTION2_FINAL_YAW_TOLERANCE_DEG\s+5\.0f" },
    @{ Name = "portion2 terminal route uses pose constrained bezier"; Pass = $guandaoC -match "PORTION2_TERMINAL_POSE_LENGTH_M\s+1\.5f" -and $guandaoC -match "portion2_shape_terminal_pose\s*\(" -and $guandaoC -match "one_minus_t\s*\*\s*one_minus_t\s*\*\s*one_minus_t" },
    @{ Name = "portion2 terminal pose uses low approach speed"; Pass = $guandaoC -match "PORTION2_TERMINAL_APPROACH_SPEED\s+3\.0f" -and $guandaoC -match "dist_to_final\s*<=\s*PORTION2_TERMINAL_POSE_LENGTH_M[\s\S]*?v_center\s*=\s*PORTION2_TERMINAL_APPROACH_SPEED" },
    @{ Name = "portion2 final missed point keeps pursuing position"; Pass = $guandaoC -notmatch "PORTION2_FINAL_MISSED_STOP_REASON" -and $guandaoC -notmatch "PORTION2_FINAL_MISSED_STOP_DIST" },
    @{ Name = "portion2 final stop aligns recorded yaw"; Pass = $guandaoC -match "PORTION2_FINAL_YAW_TOLERANCE_DEG\s+5\.0f" -and $guandaoC -match "portion2_final_yaw_align\s*\(" -and $guandaoC -match "final_point\.theta\s*-\s*Yaw_1" },
    ...
    @{ Name = "portion2 final yaw comes from recorded pose"; Pass = $guandaoC -match "recorded_terminal_yaw\s*=\s*portion2_run_reverse[\s\S]*?portion2_route_start_yaw[\s\S]*?portion2_route_final_yaw[\s\S]*?portion2_run_final_yaw\s*=\s*guandao_normalize_angle\s*\(recorded_terminal_yaw\s*\+\s*yaw_delta\)" -and $guandaoC -notmatch "portion2_run_final_yaw\s*=\s*guandao_route_point\s*\(" },
    @{ Name = "portion2 yaw alignment reacquires endpoint position"; Pass = $guandaoC -match "PORTION2_FINAL_YAW_REACQUIRE_DIST\s+0\.25f" -and $guandaoC -match "PORTION2_FINAL_YAW_ALIGN_REACQUIRE" -and $guandaoC -match "align_status\s*==\s*PORTION2_FINAL_YAW_ALIGN_REACQUIRE" },
    ...
    @{ Name = "main modes include record run and drive"; Pass = $displayH -match "typedef\s+enum\s*\{\s*Guandao_Voice\s*,\s*Guandao_Portion2_Recode\s*,\s*Guandao_Drive\s*\}\s*Mode_Choice\s*;" },
