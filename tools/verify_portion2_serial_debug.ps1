...
$checks = @(
    ...
    @{ Name = "portion2 final yaw align does not drive forward"; Pass = $guandaoC -match "PORTION2_FINAL_YAW_MISMATCH_REASON" -and $guandaoC -match "\*out_v_l\s*=\s*0\.0f" -and $guandaoC -match "\*out_v_r\s*=\s*0\.0f" -and $guandaoC -match "\*out_servo\s*=\s*0\.0f" },
    ...
    @{ Name = "portion2 final yaw align removes circling servo"; Pass = $guandaoC -notmatch "out_servo\s*=\s*\(yaw_error\s*>\s*0\.0f\)\s*\?" },
