...
$checks = @(
    ...
    @{ Name = "gps startup requires three loosely stable fixes"; Pass = $gpsC -match "PORTION2_GPS_STARTUP_REQUIRED_SAMPLES\s+\(3U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_STABILITY_M\s+\(1\.2f\)" -and $gpsC -match "PORTION2_GPS_STARTUP_MAX_SHIFT_M\s+\(5\.0f\)" },
    @{ Name = "gps fusion gates hdop and excessive startup shift"; Pass = $gpsC -match "PORTION2_GPS_FUSION_MAX_HDOP\s+\(3\.0f\)" -and $gpsC -match "gnss\.hdop\s*<=\s*0\.0f\s*\|\|\s*gnss\.hdop\s*>\s*PORTION2_GPS_FUSION_MAX_HDOP" -and $gpsC -match "startup_shift\s*>\s*PORTION2_GPS_STARTUP_MAX_SHIFT_M" },
    ...
    @{ Name = "each route stores up to seventy five inertial points"; Pass = $guandaoH -match "PORTION2_ROUTE_MAX_POINTS\s+75" -and $guandaoH -match "PORTION2_TOTAL_ROUTE_POINTS\s+900" -and $guandaoH -match "MAX_LENGTH_INDEX\s+588" -and $guandaoH -match "PORTION2_ROUTE_STORAGE_EXTENSION_POINTS\s+\(PORTION2_TOTAL_ROUTE_POINTS\s*-\s*MAX_LENGTH_INDEX\)" },
    @{ Name = "each route stores up to thirty gps points"; Pass = $guandaoH -match "PORTION2_GPS_PER_ROUTE\s+30" },
    @{ Name = "twelve routes reserve 360 gps records"; Pass = $guandaoH -match "MAX_GPS_RECODE\s+240" -and $guandaoH -match "PORTION2_TOTAL_GPS_COUNT\s+360" -and $guandaoH -match "PORTION2_GPS_STORAGE_EXTENSION_POINTS\s+\(PORTION2_TOTAL_GPS_COUNT\s*-\s*MAX_GPS_RECODE\)" },
    @{ Name = "route and gps backing arrays have compile time guards"; Pass = $guandaoH -match "PORTION2_TOTAL_ROUTE_POINTS\s*!=\s*PORTION2_ROUTE_COUNT\s*\*\s*PORTION2_ROUTE_MAX_POINTS" -and $guandaoH -match "PORTION2_ROUTE_STORAGE_EXTENSION_POINTS\s*<=\s*0" -and $guandaoH -match "PORTION2_GPS_STORAGE_EXTENSION_POINTS\s*<=\s*0" },
    ...
    @{ Name = "expanded gps table uses two flash pages"; Pass = $flashH -match "RECODE_PASSAGE_TWO" -and $flashH -match "RECODE_PASSAGE_GPS_CONT" -and $flashC -match "flash_write_page_from_buffer\s*\(\s*FLASH_SECTION_INDEX\s*,\s*RECODE_PASSAGE_GPS_CONT\s*\)" -and $flashC -match "flash_read_page_to_buffer\s*\(\s*FLASH_SECTION_INDEX\s*,\s*RECODE_PASSAGE_GPS_CONT\s*\)" },
    @{ Name = "inertial and gps flash pages have compile time guards"; Pass = $flashC -match "PORTION2_ROUTE_CONT_PAGE_WORDS\s*>\s*PORTION2_FLASH_PAGE_WORDS" -and $flashC -match "PORTION2_GPS_PRIMARY_PAGE_WORDS\s*>\s*PORTION2_FLASH_PAGE_WORDS" -and $flashC -match "PORTION2_GPS_CONT_PAGE_WORDS\s*>\s*PORTION2_FLASH_PAGE_WORDS" },
    @{ Name = "gps pages store version markers and record counts"; Pass = $flashC -match "PORTION2_GPS_LAYOUT_MAGIC\s+\(0x50324755U\)" -and $flashC -match "PORTION2_GPS_CONTINUATION_MAGIC\s+\(0x50324743U\)" -and $flashC -match "PORTION2_GPS_PAGE_TOTAL_INDEX" -and $flashC -match "PORTION2_GPS_CONT_COUNT_INDEX" },
    ...
    @{ Name = "gps startup uses bounded stability sampling"; Pass = $gpsC -match "PORTION2_GPS_STARTUP_REQUIRED_SAMPLES\s+\(3U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_TIMEOUT_MS\s+\(10000U\)" -and $gpsC -match "PORTION2_GPS_STARTUP_STABILITY_M\s+\(1\.2f\)" },
    ...
    @{ Name = "new thirty point gps layout marker invalidates old binding"; Pass = $flashC -match "PORTION2_GPS_LAYOUT_MAGIC\s+\(0x50324755U\)" },
    ...
    @{ Name = "portion2 steering limit is selected from raw route curvature"; Pass = $guandaoC -match "portion2_max_reference_turn\s*\(" -and $guandaoC -match "reference_turn\s*>=\s*PORTION2_SHARP_TURN_TRIGGER_DEG" -and $guandaoC -match "portion2_selected_route\s*==\s*PORTION2_ROUTE_SNAKE[\s\S]*?PORTION2_SNAKE_SHARP_STEERING_CMD_LIMIT\s*:\s*PORTION2_SHARP_STEERING_CMD_LIMIT" },
