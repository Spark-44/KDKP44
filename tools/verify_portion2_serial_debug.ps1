...
$checks = @(
    ...
    @{ Name = "portion2 final missed point forces stop"; Pass = $guandaoC -match "PORTION2_FINAL_MISSED_STOP_DIST\s+1\.5f" -and $guandaoC -match "PORTION2_FINAL_MISSED_STOP_REASON\s+12U" -and $guandaoC -match "state->current_point_index\s*>=\s*route_length\s*-\s*1" -and $guandaoC -match "dist_to_final\s*<=\s*PORTION2_FINAL_MISSED_STOP_DIST" },
