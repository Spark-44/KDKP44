...
Assert-Contains $main 'remote_control_task\s*\(\s*\)\s*;\s*Record_Idle_Encoder_Diag_Update\s*\(\s*\)\s*;\s*Rear_Motor_Serial_Telemetry_Update\s*\(\s*\)\s*;' 'Encoder diagnostics must run after remote task and before slower telemetry.'
