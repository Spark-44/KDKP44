...
Assert-Contains $header '#define\s+ANGLE_DEFAULT_KP\s+500\.0f' 'Angle Kp must match the migrated controller.'
Assert-Contains $header '#define\s+ANGLE_DEFAULT_KI\s+18\.0f' 'Angle Ki must match the migrated controller.'
Assert-Contains $header '#define\s+ANGLE_DEFAULT_KD\s+27\.0f' 'Angle Kd must match the migrated controller.'
...
Assert-Contains $header '#define\s+ANGLE_FEED_FORWARD\s+65\.0f' 'Angle feed-forward must match the migrated controller.'
Assert-Contains $header '#define\s+ANGLE_INTEGRAL_MAX\s+1000\.0f' 'Angle integral maximum must remain 1000.0f.'

Assert-Contains $source 'PID_Init\([\s\S]*?ANGLE_OUTPUT_MAX\s*,\s*ANGLE_INTEGRAL_MAX\s*\)' 'Angle control must pass its integral limit into PID init.'
