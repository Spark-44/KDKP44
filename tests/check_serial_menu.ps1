...
Assert-Contains $source 'Mode_Choice' 'serial menu must expose mode selection.'
Assert-Contains $source 'Run_Route' 'serial menu must expose run route selection.'
Assert-Contains $source 'Record_Control' 'serial menu must expose record controls.'
Assert-Contains $source 'Drive_Control' 'serial menu must expose drive controls.'
...
Assert-Contains $source 'portion2_run_select_route\s*\(\s*serial_menu_cursor\s*\)' 'serial menu must run the selected route from the route page.'
Assert-Contains $source 'portion2_record_select_route\s*\(\s*serial_menu_cursor\s*\)' 'serial menu must select record routes directly.'
Assert-Contains $source 'portion2_record_remote_start_stop_request\s*\(\s*\)' 'serial menu must start/stop recording through the same request path as remote buttons.'
Assert-Contains $source 'portion2_record_remote_clear_request\s*\(\s*\)' 'serial menu must clear current route through the same request path as remote buttons.'
Assert-Contains $source 'portion2_record_remote_save_request\s*\(\s*\)' 'serial menu must save current route through the same request path as remote buttons.'
Assert-Contains $source 'key1_flag\s*=\s*1' 'serial menu drive page must emulate K1 for speed down.'
Assert-Contains $source 'key2_flag\s*=\s*1' 'serial menu drive page must emulate K2 for speed up.'
